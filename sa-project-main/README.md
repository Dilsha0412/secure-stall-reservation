...
`database_schema.sql` (also available at `backend/src/main/resources/schema.sql`).
...
All configurable application parameters (database credentials, OIDC IdP settings, JWT secrets, HTTPS SSL options, and mail server credentials) are externalized in `backend/src/main/resources/application.properties`.

Open `backend/src/main/resources/application.properties` and update the parameters according to your environment:
...
2. Build the Maven project:
   ...
   ```
   *(Or run using Maven Wrapper: `./mvnw clean package -DskipTests`)*
